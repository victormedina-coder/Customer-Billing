...
import { WindowClosedScreen } from './_components/WindowClosedScreen'
...
interface PortalClientProps {
  /** true si la ventana de facturación del mes en curso ya cerró (corte 21:00 MX,
   *  calculado server-side en page.tsx con isPortalWindowOpen). Cuando está
   *  cerrada, el flujo completo (stepper + pasos) NO se renderiza — el
   *  bloqueo real vive en el servidor (lookup/emit rechazan con `deadline`);
   *  esta pantalla es la comunicación, no la seguridad. */
  windowClosed: boolean
  /** Correo de facturación configurado server-side (BILLING_CONTACT_EMAIL). */
  contactEmail?: string
}

export function PortalClient({ windowClosed, contactEmail }: PortalClientProps) {
  ...
  // Ventana de facturación cerrada para TODOS (corte 21:00 MX): pantalla de
  // bloqueo en lugar del stepper. Flag server-authoritative (page.tsx) — no
  // depende de hidratación de sessionStorage, así que se resuelve antes.
  if (windowClosed) {
    return (
      <>
        ...
        <main style={{ flex: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px 72px' }}>
          <WindowClosedScreen contactEmail={contactEmail} />
        </main>
      </>
    )
  }
